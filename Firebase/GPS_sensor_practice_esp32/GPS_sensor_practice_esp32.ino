...
void loop() {
   ...
   if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    Serial.printf("Get latitude... %s\n", Firebase.getFloat(fbdo, F("/test/latitude")) ? String(fbdo.to<float>()).c_str() : fbdo.errorReason().c_str());

    Serial.printf("Set longitude... %s\n", Firebase.setFloat(fbdo, F("/test/longitude"), longitude) ? "ok" : fbdo.errorReason().c_str());

    Serial.printf("Get longitude... %s\n", Firebase.getFloat(fbdo, F("/test/longitude")) ? String(fbdo.to<float>()).c_str() : fbdo.errorReason().c_str());
