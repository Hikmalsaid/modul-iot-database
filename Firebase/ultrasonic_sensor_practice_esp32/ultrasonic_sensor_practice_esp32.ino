...
void loop() {
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    Serial.printf("Set int... %s\n", Firebase.setInt(fbdo, F("/test/jarak"), distanceCM) ? "ok" : fbdo.errorReason().c_str());

    Serial.printf("Get int... %s\n", Firebase.getInt(fbdo, F("/test/jarak")) ? String(fbdo.to<int>()).c_str() : fbdo.errorReason().c_str());
