...
void loop() {
   ...
   if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    Serial.printf("Set analog... %s\n", Firebase.setInt(fbdo, F("/test/sensor_analog"), IRvalueA) ? "ok" : fbdo.errorReason().c_str());

    Serial.printf("Set digital... %s\n", Firebase.setInt(fbdo, F("/test/sensor_digital"), IRvalueD) ? "ok" : fbdo.errorReason().c_str());
    
    if(IRvalueA > 500)
    {
    Serial.printf("Set analog... %s\n", Firebase.setString(fbdo, F("/test/kondisi_analog"), "Tidak Terdeteksi") ? "ok" : fbdo.errorReason().c_str());    
    }else
    {
    Serial.printf("Set analog... %s\n", Firebase.setString(fbdo, F("/test/kondisi_analog"), "Terdeteksi") ? "ok" : fbdo.errorReason().c_str());    
    }
