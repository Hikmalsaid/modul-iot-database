#define DO_PIN 23  // ESP32's pin GPIO13 connected to DO pin of the ldr module
...
void loop() {
  ...
  if (lightState == HIGH)
  {
    ...
    Serial.printf("Set kondisi... %s\n", Firebase.setString(fbdo, F("/test/kondisi_sekitar"), "GELAP") ? "ok" : fbdo.errorReason().c_str());
  }
  else
  {
    Serial.printf("Set kondisi... %s\n", Firebase.setString(fbdo, F("/test/kondisi_sekitar"), "TERANG") ? "ok" : fbdo.errorReason().c_str());
